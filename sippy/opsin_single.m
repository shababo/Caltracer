...
for d=1:length(workspace)
    ...
    if isempty(stim_art_index)
        ...
        for i = 1:number_trials
            ...
            indices{i}(1:number_stims,1) = stim_art_index(s:s + number_stims -1,2) + 50;
            indices{i}(1:number_stims,2) = indices{i}(1:number_stims,1) + 500;
            event_index = indices{i};
            bl = mean(data(event_index(1,1)- bt:event_index(1,1) - 100));
            data= data -bl;
            bl = mean(data(event_index(1,1)- bt:event_index(1,1) - 100));
            data_trial{i,d} = data(event_index(1)- bt:event_index(1,2) +bt);
            for ii = 1:size(event_index,1)              
...
figure;
a(1) = subplot(2,1,1); hold on; title('amp');
...
a(2) = subplot(2,1,2); hold on; title('integral');
