...
load('C:\Tanya_Data\Imaging\workspace\042411_s1r2_9.mat');
...
end_frame = 700;
frametime = frametime(1:end_frame);


UP_start_user_defined = 0; % use this if have spont activity or UP doesn't start right after stim
start_UP_index_USER = 600;
end_UP_index_USER = start_UP_index_USER + 200;

F = -detrend(rawF(1:end_frame));
F=F-min(F); 
F=F/max(F); 
F=F+eps;
...
if UP_start_user_defined == 1;
    UP_index = [start_UP_index_USER:end_UP_index_USER];
else
...
for nzt = 1:length(nonzero_thresh)
    ...
    spike_index = spike_index(find(spike_index < end_frame));
    if spike_index(1) ==1;
        spike_index(1) = [];
    end
